            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <div class="d-flex flex-wrap">
                <div class="p-2 flex-fill">
                  <!-- page title -->
                  <h4 class="fw-bold p-2"><span class="text-muted fw-light">Faculty /</span> Manage Faculties</h4>
                </div>
                <div class="p-2 flex-fill">
                  <!-- search -->
                  <?php 
                    include "db_conn.php";
                    $sql = "SELECT * FROM faculty_tbl ORDER BY lname ASC";

                    if (isset($_POST['search'])) {
                      $search = $_POST['search'];
                      $sql = "SELECT * FROM faculty_tbl WHERE CONCAT(fname, ' ', lname) LIKE '%$search%' OR CONCAT(lname, ' ', fname) LIKE '%$search%'";
                    }
                    else {
                      $search = "";
                      $sql = "SELECT * FROM faculty_tbl ORDER BY lname ASC";
                  ...
                  ?>
                  <form method="POST">
                    <label>Search</label><input type="text" class="search" placeholder="Name..." name="search" value="<?php echo $search;?>">
                  </form>
                </div>
              </div>
              <div class="container-xxl p-4 card">
                <div class="row gap-4">
                  <?php
                    if($result->num_rows > 0) {
                      while ($row=$result->fetch_assoc()) {
                  ?>
                  <div class="col-md-2 p-0" style="border: 1px solid #cccccc;">
                    <img src="faculties/<?php echo $row['profile_pic'];?>" class="img-fluid">
                    ...
                    <div class="col p-2">
                      <div class="d-flex flex-wrap justify-content-center gap-2">
                          <a href="upd_faculty.php?faculty_id=<?=$row['faculty_id'];?>"><button class="editbtn">Update</button></a>
                          <a href="view_facultyDetails.php?faculty_id=<?=$row['faculty_id'];?>"><button class="viewbtn">View</button></a>
                      </div>
                    </div>
                  </div>
                  <?php
                      ...
                      echo "<center><span style='color: red'>Member not found</span></center>";
                    }
                    else {
                      echo "<center><span style='color: red'>No members available</span></center>";
                    }
                  ?>
                </div>
              </div>
