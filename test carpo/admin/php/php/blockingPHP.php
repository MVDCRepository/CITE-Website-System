	...
	if (isset($_POST['add_blockBtn'])) {
		...
		foreach ($_POST['subj_id'] as $key => $value) {
			if (in_array($_POST['subj_id'][$key], $checked_array)) {
				...
				$sql = "INSERT INTO blocking_tbl (block_no, yr_lvl, sem, acad_yr, total_student, eval_id, subj_id, start_time, end_time, days) VALUES ('$block_no', '$yr_lvl', '$sem', '$acad_yr', '$total_student', '$eval_id', '$subj_id', '$start_time', '$end_time', '$days')";
				$sql_result = mysqli_query($conn, $sql);
			}
		}
